...
const ReservationManagement: React.FC = () => {
  ...
  const statusSummary = ['pending', 'confirmed', 'cancelled', 'no_show'].map((status) => ({
    key: status,
    label: status === 'no_show' ? 'No Show' : status.charAt(0).toUpperCase() + status.slice(1),
    count: reservations.filter((reservation: any) => normalizeStatusValue(reservation.status) === status).length,
  }));
  ...
  const pagedReservations = filteredReservations.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
  const pageCount = Math.ceil(filteredReservations.length / ITEMS_PER_PAGE);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Table Booking Status</h2>
            <p className="text-sm text-gray-500">A quick overview of booking activity and follow-up needs.</p>
          </div>
          <div>
              ...
              {pagedReservations.map((res: any) => (
                <tr key={res.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[normalizeStatusValue(res.status)]}`}>
                      {normalizeStatusValue(res.status)}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <select className="text-sm border border-gray-300 rounded px-2 py-1" value={normalizeStatusValue(res.status)} onChange={(e) => updateReservationStatus(res.id, e.target.value as any)}>
                      <option value="pending">Pending</option>
                      <option value="confirmed">Confirmed</option>
                      <option value="cancelled">Cancelled</option>
                      <option value="no_show">No Show</option>
