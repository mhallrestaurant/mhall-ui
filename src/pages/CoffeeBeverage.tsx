...
const CoffeeBeveragePage = () => {
  ...
  const { items: dbItems, loading } = useMenuItems('COFFEE')
        ...
        <div className="max-w-7xl mx-auto mb-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-16">
            <div className="rounded-3xl overflow-hidden shadow-2xl hover:shadow-3xl transition-shadow duration-500">
              <img src={menuItems[0]?.image || menuItems[1]?.image || ''} alt="Artisanal Coffee Experience" className="w-full h-80 lg:h-96 object-cover" />
            </div>
            <div className="rounded-3xl overflow-hidden shadow-2xl hover:shadow-3xl transition-shadow duration-500">
              <img src={menuItems[1]?.image || menuItems[0]?.image || ''} alt="Premium Beverage Selection" className="w-full h-80 lg:h-96 object-cover" />
            </div>
          </div>
          {/* Menu Section */}
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-500 text-xl">No items available.</div>
          ) : (
